...
member_id = sys.argv[1] if len(sys.argv) > 1 else uuid.uuid4().hex[:6]
...
def send_text():
    while True:
        ...
        if text.lower() == 'exit':
            print(f"[Member {member_id}] Exiting...")
            os._exit(0)
        payload = f"{member_id}:{text}"
        text_pub.send_multipart([f"{topics[2]}:{member_id}".encode(), payload.encode()])
