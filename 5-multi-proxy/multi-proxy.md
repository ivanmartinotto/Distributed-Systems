...
This project demonstrates a multi-broker architecture using the ZeroMQ (ZMQ) Python library. It implements a publisher-subscriber model with multiple brokers, each handling a different topic to emulate text, audio, and video streams.
...
- **Subscribers**: Two subscribers are implemented, each subscribing to one distinct topic to receive and process the corresponding messages.
